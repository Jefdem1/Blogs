...
myAWSIoTMQTTClient.configureCredentials('./root-CA.crt', './PiTemp1.private.key', './PiTemp1.cert.pem')
...
def gettemp():
	...
	if len(lengths) != 40:
		...
		if the_bytes[4] != checksum:
			...
			th["temp"] = (9.0 / 5.0 * int(the_bytes[2]) + 32)
			th["humidity"] = the_bytes[0]
...
def getgas():
    ...
    rr_ratio = rs/Ro
    gas["GAS_LPG"] = (math.pow(10,(((math.log(rr_ratio)-LPGCurve[1])/LPGCurve[2])+LPGCurve[0])))
    gas["CO"] = (math.pow(10,(((math.log(rr_ratio)-COCurve[1])/ COCurve[2]) + COCurve[0])))
    gas["SMOKE"] = (math.pow(10,(((math.log(rr_ratio)-SmokeCurve[1])/SmokeCurve[2])+SmokeCurve[0])))
	return gas
...
while True:
	...
	while (len(th) == 0):
		th = gettemp()
	output = '{'
	output += '"Temperature":"' + str(th["temp"]) + '",'
	output += '"Humidity":"' + str(th["humidity"]) + '",'
	output += '"TTL":' + str(int(time.time() + 86400)) + ','
	output += '"Timestamp":' + str(int(time.time())) + ','
	output += '"Location":"Outside",'
	output += '"LPG":"' + str(gas["GAS_LPG"]) + '",'
	output += '"CO":"' + str(gas["CO"]) + '",'
	output += '"Smoke":"' + str(gas["SMOKE"]) + '"'
	output += '}'
	#iotresults = myAWSIoTMQTTClient.publish('sdk/test/Python', output, 1)
	print(output + "(Posted: " + str(iotresults))
	time.sleep(9)
